...
namespace PudelkoLibrary
{
    public sealed class Pudelko : IFormattable, IEquatable<Pudelko>, IEnumerable<double>, IComparable<Pudelko>
    {
        ...
        public static bool operator ==(Pudelko pudelko, Pudelko pudelko2) => pudelko.Equals(pudelko2);
        public static bool operator !=(Pudelko pudelko, Pudelko pudelko2) => !pudelko.Equals(pudelko2);
        ...
        public static Pudelko operator +(Pudelko pudelko, Pudelko pudelko2)
        {
            double[] doublePudelko = (double[])pudelko, doublePudelko2 = (double[])pudelko2;

            Array.Sort(doublePudelko);
            Array.Sort(doublePudelko2);

            return new Pudelko(
                doublePudelko[0] + doublePudelko2[0],
                doublePudelko[1] + doublePudelko2[1],
                doublePudelko[2] + doublePudelko2[2]
            );
        }
        ...
        {
            double objetoscPudelka = this.Objetosc;
            double objetoscPudelka2 = pudelko.Objetosc;

            if (objetoscPudelka == objetoscPudelka2)
            {
                double pole = this.Pole;
                double polePudelko2 = pudelko.Pole;

                if (pole == polePudelko2)
                {
                    double sumPudelko = this.A + this.B + this.C;
                    double sumPudelko2 = pudelko.A + pudelko.B + pudelko.C;

                    if (sumPudelko == sumPudelko2)
                        return 0;

                    return sumPudelko < sumPudelko2 ? 1 : -1;
                }

                return pole < polePudelko2 ? 1 : -1;
            }

            return (objetoscPudelka < objetoscPudelka2) ? 1 : -1;
        }
    }
}